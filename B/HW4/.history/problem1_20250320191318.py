...
# Bisection search for problem 1

left = 0
right = len(results_seigs) - 1
while right - left > 1:
    mid = (left + right) // 2
    if results_seigs[mid] * results_seigs[right] < 0:
        left = mid
    else:
        right = mid

print(f'The bifurcation point is r = {r_space[right]}')
    

plt.plot(r_space, results_seigs, marker='x', label='eigenvalues with smallest abs real part')
# plt.plot(r_space, results_leigs, marker='o', label='eigenvalues with largest real part')
plt.scatter(r_space[right], results_seigs[right], color='r', label='bifurcation point')
