...
def F(U, h, N, b2, r):
    ...
    F[N:] = 3 *  (M@(r*u + b2 * u**2 - u**3) - (A + M) @ v).flatten()  
    return F
...
def J(U, h, N, b2, r):
    ...
    J[:N, :N] = A + M
    J[:N, N:] = - M
    J[N:, :N] = 3 *  M @ diags((r + 2 * b2 * u - 3 * u**2).flatten(), offsets=0, format="csr")
    J[N:, N:] = - (A + M)
...
eigvalsm, eigvecm = spla.eigs(J, k=8, sigma=0)

print("eigvalsm = ", eigvalsm)