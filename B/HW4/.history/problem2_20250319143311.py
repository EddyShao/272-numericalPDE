...
# Define the FEM function
def FEM(U, h, N, b2, r):
    e = np.ones(N)
    A = (1/h) * sp.diags([-e, 2*e, -e], [-1, 0, 1], shape=(N, N), format="csr")
    A[0, -1] = -1/h
    A[-1, 0] = -1/h

    M = (h/6) * sp.diags([e, 4*e, e], [-1, 0, 1], shape=(N, N), format="csr")
    M[0, -1] = h/6
    M[-1, 0] = h/6
...
def jacobian(U, h, N, b2, r):
    ...
    J11 = M + A
    J12 = -M
    J21 = -M @ sp.diags(r + 2*b2*u - 3*u**2, 0)
    J22 = M + A

    J_top = sp.hstack([J11, J12])
    J_bottom = sp.hstack([J21, J22])
    return sp.vstack([J_top, J_bottom], format="csr")
...
def newton_krylov(F, U0, tol, max_iter, h, N, b2, r):
    ...
    for k in range(max_iter):
        ...
        delta_U, _ = spla.gmres(J, -Fx, tol=1e-6, maxiter=130)
        
        alpha = 1.0
        for _ in range(10):
            U_temp = U + alpha * delta_U
            if np.linalg.norm(F(U_temp, r)) < np.linalg.norm(Fx):
                U = U_temp
                break
            alpha *= 0.5
    return U, residuals
...
def compute_tangent(U, r, h, N, b2, tangent_prev):
    ...
    tangent_temp = np.hstack([dU_temp, dr_prev])

    A = np.vstack([sp.vstack([J, Fr]), np.hstack([tangent_temp, 0])])
    b = np.hstack([np.zeros(2*N), 1])
