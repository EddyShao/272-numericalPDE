...
def JVP(U, v):
    """Computes Jacobian-vector product J(U) * v using the assembled Jacobian"""
    J_U = J(U, h, N, b2, r)  # Compute the Jacobian at U
    return J_U @ v  # Matrix-vector product

# Define the linear operator for JVP
J_op = LinearOperator((2*N, 2*N), matvec=lambda v: JVP(U_0, v))
...
U_sol = newton_krylov(lambda U: F(U, h, N, b2, r), U_0, f_tol=TOL)
u = U_sol[:N]
