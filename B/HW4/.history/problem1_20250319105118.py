...
eigvals, eigvecs = spla.eigs(J, k=J.shape[0] - 2)  # Get all (or a large subset)

# Find eigenvalue with the smallest absolute real part
idx = min(range(len(eigvals)), key=lambda i: abs(eigvals[i].real))
eigvalsm, eigvecm = eigvals[idx], eigvecs[:, idx]