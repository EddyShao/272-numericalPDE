...
print(v(u_0 + 1e-4 * phi.real.flatten).shape)

U_right = np.concatenate((u_0 + 1e-4 * phi, v(u_0 + 1e-4 * phi.real)))
res_right = F_res(U_right, h, N, b2, r=1.0)

U_left = np.concatenate((u_0 - 1e-4 * phi.real, v(u_0 - 1e-4 * phi.real)))
res_left = F_res(U_left, h, N, b2, r=1.0)

alpha = np.dot(psi.real, (res_right - res_left) / 2e-4) 
print(alpha)
