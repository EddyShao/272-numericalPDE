...
# ---- Step 1: Compute critical eigenvector φ and adjoint eigenvector ψ ----
    J_star = Jacobian(u_current, r_current)
    φ = solve_eigenproblem(J_star, eigenvalue=0)    # J_star * φ = 0
    ψ = solve_eigenproblem_transpose(J_star^T, eigenvalue=0)  # J_star^T * ψ = 0
    normalize(φ, ψ)  # e.g. ensure dot(φ, ψ) = 1
