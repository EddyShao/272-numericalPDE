...
# assemble augmented jacobian matrix
J_aug = lil_matrix((2*N + 1, 2*N + 1))
J_aug[:2*N, :2 * N] = J
J_aug[2*N, :N] = u_0.T
J_aug[2*N, N:2*N] = v_0.T
J_aug[N:2*N, 2*N] = -M@u_0 

# compute the eigenvalues of the augmented jacobian matrix
eigvals = spla.eigs(J_aug, k=2*N, return_eigenvectors=False)
print("Eigenvalues of the augmented Jacobian matrix: ", eigvals
