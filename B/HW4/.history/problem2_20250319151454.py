...
def compute_tangent_vector(U, h, N, b2, r):
    ...
    J_U = J(U, h, N, b2, r)  # Compute the Jacobian
    ...
    A = np.vstack([np.hstack([J_U.toarray(), F_r[:, None]]), np.concatenate([x_star, np.array([0])])])
...
beta = 0 # beta is the scale of the initial u
u_0 = beta * np.sin(2 * np.pi * x / L)
v_0 = - (2 * np.pi/L)**2 * u_0 + u_0  # v_0 =  \partial^{2} u / \partial x^{2} + u
U_0 = np.concatenate((u_0, v_0))

solutions, params = arclength_continuation(U_0, h, N, b2, r, ds=0.1, num_steps=4)
