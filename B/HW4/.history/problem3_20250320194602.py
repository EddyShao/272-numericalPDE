...
##############################################################################
# 1) PDE Residual Assembly
##############################################################################
def assemble_pde_residual(u, r, D, N, dx):
    """
    Build the PDE residual F(u,r) for the 1D PDE:
        F(u,r) = r*u + D*u_xx - u^3
    with periodic boundary conditions.
    
    u: 1D array of length N, representing the solution on grid points
    r: scalar parameter
    D: diffusion coefficient
    N: number of grid points
    dx: grid spacing
    Returns: F(u,r) in R^N
    """
    # Finite difference for second derivative (periodic)
    # Example: u_xx ~ (u[i+1] - 2u[i] + u[i-1]) / dx^2
    # We'll build that in a loop or a matrix-based approach:
    u_xx = np.zeros_like(u)
    for i in range(N):
        i_left = (i - 1) % N
        i_right = (i + 1) % N
        u_xx[i] = (u[i_right] - 2*u[i] + u[i_left]) / (dx**2)

    # Now build F(u,r):
    F_u = r*u + D*u_xx - u**3
    return F_u

def assemble_jacobian(u, r, D, N, dx):
    """
    Sparse Jacobian J(u,r) = dF/du, size NxN.
    For F(u,r) = r*u + D*u_xx - u^3,
    dF/du = r*I + D*L - diag(3*u^2),
    where L is the discrete Laplacian matrix (periodic).
    """
    # Discrete Laplacian (periodic) in sparse form:
    e = np.ones(N)
    # Tridiagonal: -2 on main diag, 1 on off diag, plus wrap for periodic
    diag_main = -2*e
    diag_off = e
    L = sp.diags([diag_off, diag_main, diag_off], [-1, 0, 1], shape=(N,N)).tolil()
    # Periodic wrap
    L[0, N-1] = 1
    L[N-1, 0] = 1
    L = L.tocsr() / (dx**2)
    ...
    # Combine to get J:
    # r*I + D*L - diag(3*u^2)
    diag_cubic = sp.diags([ -3*(u**2) ], [0])
    J = r*sp.eye(N) + D*L - diag_cubic
    return J

##############################################################################
# 2) Simple Newton Solve to Find a Branch
##############################################################################
def newton_solve(u_init, r, D, N, dx, tol=1e-8, max_iter=50):
    """
    Solve F(u,r)=0 using Newton. Returns (u_sol).
    """
    u = u_init.copy()
    for _ in range(max_iter):
        F_u = assemble_pde_residual(u, r, D, N, dx)
        res_norm = np.linalg.norm(F_u)
        if res_norm < tol:
            break
        J = assemble_jacobian(u, r, D, N, dx)
        # Solve J * delta = -F_u
        delta, info = spla.cg(J, -F_u, tol=1e-8)
        # Simple line search
        alpha = 1.0
        for __ in range(10):
            u_test = u + alpha*delta
            F_test = assemble_pde_residual(u_test, r, D, N, dx)
            if np.linalg.norm(F_test) < res_norm:
                u = u_test
                break
            alpha *= 0.5
    return u
...
##############################################################################
# 3) Find Bifurcation: We assume there's a trivial solution (u=0).
#    In your real problem, you'd do a parameter sweep & check eigenvalues.
##############################################################################
